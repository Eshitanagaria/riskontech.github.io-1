document.addEventListener('DOMContentLoaded', function () {
                     ...
                     <div class="report-section mt-6">
                        ...
                        <div class="report-section-content">
                            ...
                            <div id="pdf-download-area" class="hidden mt-4">
                               <button id="download-pdf-btn" onclick="downloadReportAsPDF('${applicantId}')" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-full transition duration-300">
    ...
    // PDF GENERATION FIX
    window.downloadReportAsPDF = function(applicantId) {
        ...
        const pdfBtn = elementToPrint.querySelector('#pdf-download-area');
        if (pdfBtn) pdfBtn.style.display = 'none';
        
        // Handle the canvas chart for PDF
        const canvas = elementToPrint.querySelector('#interactive-graph-canvas');
        if(canvas && currentChart) {
            const image = new Image();
            image.src = currentChart.toBase64Image();
